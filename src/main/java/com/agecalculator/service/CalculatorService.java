...
public class CalculatorService {
    ...
    public CalculationResult calculate(double num1, double num2, char operator) {
        switch (operator) {
            case '+':
                // Addition operation: sum of the two operands
                return new CalculationResult(num1, num2, operator, num1 + num2);
            case '-':
                // Subtraction operation: difference of the two operands
                return new CalculationResult(num1, num2, operator, num1 - num2);
            case '*':
                // Multiplication operation: product of the two operands
                return new CalculationResult(num1, num2, operator, num1 * num2);
            case '/':
                // Division with zero-check guard before operation
                if (num2 == 0) {
                    throw new ArithmeticException("Cannot divide by zero");
                }
                return new CalculationResult(num1, num2, operator, num1 / num2);
            case '%':
                // Modulus with zero-check guard before operation
                if (num2 == 0) {
                    throw new ArithmeticException("Cannot divide by zero");
                }
                return new CalculationResult(num1, num2, operator, num1 % num2);
            default:
                // Unsupported operator: reject with descriptive error message
